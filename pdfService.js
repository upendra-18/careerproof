...
/**
 * addAicteMark
 * Uses the REAL AICTE logo (aicte-logo.png in assets/).
 * Falls back to a drawn version only if file is missing.
 */
function addAicteMark(doc, x, y, size) {
  const logoPath = path.join(__dirname, '..', 'assets', 'aicte-logo.png');
  ...
  // ⚠️  Fallback only — runs if aicte-logo.png is missing from assets/
  console.warn('aicte-logo.png not found — using drawn fallback');
  doc.save();
...
function addSignature(doc, name, role, x, y, width, imageName) {
  ...
  if (signaturePath && fs.existsSync(signaturePath)) {
    doc.image(signaturePath, x+8, y, { fit: [width-16, 26], align:'center', valign:'center' });
  } else {
...
function addCertificateBrand(doc, y) {
  ...
  const cw = doc.widthOfString('CAREER', { characterSpacing: spacing });
  const pw = doc.widthOfString('PROOF',  { characterSpacing: spacing });
  const startX = doc.page.width/2 - (cw + pw + 8)/2;
  doc.fillColor('#141b3d').text('CAREER', startX, y, { characterSpacing: spacing });
  doc.fillColor('#00a977').text('PROOF', startX + cw + 8, y, { characterSpacing: spacing });
}

function textBlock(doc, text, x, y, width, options = {}) {
  doc.text(text, x, y, { width, align: options.align || 'justify', lineGap: options.lineGap ?? 2 });
  return doc.y;
}
...
async function generateOfferLetterPDF(applicantData) {
  ...
  doc.font('Helvetica-Bold').fontSize(6).fillColor('#333333').text('VERIFY STUDENT ID', W-138, 90, { width:90, align:'center' });
  doc.font('Helvetica-Bold').fontSize(24).fillColor('#111111').text('INTERNSHIP OFFER LETTER', cX, 95, { width:cW, align:'center' });
  ...
  doc.font('Helvetica-Bold').fontSize(8.5).fillColor('#111111').text('Important:', cX+14, 576, { continued:true });
  doc.font('Helvetica').fontSize(8.5).text(' Timely task submission and professional conduct are mandatory to receive the internship completion certificate.', { width:cW-28 });
  doc.font('Helvetica-Bold').fontSize(8.6).fillColor('#333333').text('We congratulate you on your selection and look forward to your active participation in the CareerProof Internship Program.', cX, 612, { width:cW });
...
async function generateCertificatePDF(applicantData) {
  ...
  if (fs.existsSync(logoPath)) {
    doc.save();
    doc.opacity(0.18).image(logoPath, PW/2-180, 116, { width:360 });
  ...
  doc.font('Helvetica-Bold').fontSize(6).fillColor('#141b3d').text('VERIFY', PW-125, 116, { width:74, align:'center' });

  addCertificateBrand(doc, 46);
  doc.font('Times-Roman').fontSize(18).fillColor('#444444').text('T H I S  A C K N O W L E D G E S  T H A T', 0, 102, { width:PW, align:'center' });
  doc.font('Helvetica-Bold').fontSize(34).fillColor('#141b3d').text(fullName.toUpperCase(), 90, 142, { width:PW-180, align:'center' });
  doc.font('Times-Roman').fontSize(16).fillColor('#555555').text('has successfully completed the', 0, 192, { width:PW, align:'center' });
  doc.font('Helvetica-Bold').fontSize(24).fillColor('#111111').text('C E R T I F I C A T E  O F  I N T E R N S H I P', 0, 230, { width:PW, align:'center' });
  ...
  addIsoMark  (doc, PW/2-50, 374, 100);
